...
<div style="margin: auto;text-align: center;">
    <h3 style="margin: auto">处理结果</h3>
	<p></p>
    <table style="border:1px solid;align: center;margin: auto;text-align: center;">
        <tr>
        	<td style="width: 60px;border:1px solid;">配方id</td>
            <td style="width: auto;border:1px solid;">公司名称</td>
            <td style="width: auto;border:1px solid;">配方比例</td>
            <td style="width: 60px;border:1px solid;">总价格</td>
            <td style="width: 70px;border:1px solid;">操作</td>
        </tr>
        <c:forEach items="${pflist}" var="S">
        <tr>
            <td style="width: 60px;border:1px solid;">${S.id }</td>
            <td style="width: auto;border:1px solid;">${S.conpanyname }</td>
			<c:if test="${fn:length(S.peibi ) == 1}"><td style="width: auto;border:1px solid;">全部</td></c:if>
			<c:if test="${fn:length(S.peibi ) != 1}"><td style="width: auto;border:1px solid;">${S.peibi }</td></c:if>
            <td style="width: 60px;border:1px solid;">${S.summoney }</td>
            <td style="width: 70px;border:1px solid;"><a href="${pageContext.request.contextPath}/Demo/xiangqingpage?id=${S.id }&fenShu=${fenShu}"><input type="button" value="查看详情" ></a></td>
        </tr>
        </c:forEach>
    </table>
</div>
